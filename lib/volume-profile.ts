export type VolumeCandle = { time: number; high: number; low: number; volume: number };
...
export function buildVolumeProfile(candles: VolumeCandle[], from: number, to: number, rows = 24) {
  ...
  const bins = Array.from({ length: count }, (_, i) => ({ low: low + i * step, high: low + (i + 1) * step, volume: 0 }));
  for (const candle of selected) {
    if (candle.high === candle.low) {
      bins[Math.max(0, Math.min(count - 1, Math.floor((candle.low - low) / step)))].volume += candle.volume;
    } else {
      for (const bin of bins) bin.volume += candle.volume * Math.max(0, Math.min(candle.high, bin.high) - Math.max(candle.low, bin.low)) / (candle.high - candle.low);
    }
  }
  return bins;
}
