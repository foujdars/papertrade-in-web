...
export type HomeAttention = {id:string;title:string;detail:string;tone:'warning'|'info';target:{kind:'position';symbol:string;product:'INTRADAY'|'DELIVERY'}|{kind:'alerts';tab:'list'|'log';id?:string}};
...
export function positionAttention(positions:Array<{symbol:string;product:'INTRADAY'|'DELIVERY';side:string;quantity:number}>,protections:PaperProtection[]):HomeAttention[]{
 ...
 const p=missing[0];return p?[{id:`position:${p.symbol}:${p.product}`,title:`${p.symbol} · No recorded stop-loss`,detail:`${p.product==='INTRADAY'?'Intraday':'Delivery / carry'} · ${p.quantity} units${missing.length>1?` · ${missing.length-1} more without a stop`:''}`,tone:'warning',target:{kind:'position',symbol:p.symbol,product:p.product}}]:[];
}
