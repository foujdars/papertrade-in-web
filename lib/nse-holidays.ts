// NSE capital-market circular NSE/CMTR/71775 (not the settlement calendar).
// https://nsearchives.nseindia.com/content/circulars/CMTR71775.pdf
// Live dated exchange sessions take precedence, including Muhurat/special sessions.
export const NSE_HOLIDAYS: Record<string, string> = {
  "2026-01-26": "Republic Day", "2026-03-03": "Holi",
  "2026-03-26": "Shri Ram Navami", "2026-03-31": "Shri Mahavir Jayanti",
  "2026-04-03": "Good Friday", "2026-04-14": "Dr. Baba Saheb Ambedkar Jayanti",
  "2026-05-01": "Maharashtra Day", "2026-05-28": "Bakri Id",
  "2026-06-26": "Muharram", "2026-09-14": "Ganesh Chaturthi",
  "2026-10-02": "Mahatma Gandhi Jayanti", "2026-10-20": "Dussehra",
  "2026-11-08": "Diwali Laxmi Pujan", "2026-11-10": "Diwali-Balipratipada",
  "2026-11-24": "Guru Nanak Jayanti", "2026-12-25": "Christmas",
};
