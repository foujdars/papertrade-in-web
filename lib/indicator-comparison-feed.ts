...
export function useComparisonCandles(keys:string[],timeframe:string){
 const key=JSON.stringify([...new Set(keys)].sort()),scope=timeframe+key;
 ...
 useEffect(()=>{
  ...
  async function refresh(){if(disposed||running||document.hidden||!requested.length)return;running=true;
   const entries=await Promise.all(requested.map(async instrumentKey=>{const controller=new AbortController();controllers.add(controller);const timer=window.setTimeout(()=>controller.abort(),15000);
    try{const params=new URLSearchParams({instrumentKey,timeframe}),response=await fetch(`/api/upstox/candles?${params}`,{cache:'no-store',signal:controller.signal}),body=await response.json();if(!response.ok||!body.ok||!Array.isArray(body.candles))throw Error('No comparison data');
