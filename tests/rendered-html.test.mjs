...
test("ships project assets and removes the starter preview", async () => {
  ...
  assert.match(styles, /pnl-calendar-day\.profit/);
  assert.match(styles, /pnl-calendar-day\.loss/);
  assert.match(styles, /pnl-calendar-day\.no-trade/);
  assert.match(styles, /pnl-calendar-day\.holiday/);
  ...
  const studyCatalog = await readFile(new URL("../lib/indicator-catalog.ts", import.meta.url), "utf8");
  assert.match(functionMenu, /STUDIES\.filter/);
  assert.match(studyCatalog, /VWAP/);
  assert.match(studyCatalog, /MACD/);
  assert.match(studyCatalog, /SuperTrend/);
  assert.match(studyCatalog, /Pivot Points Standard/);
