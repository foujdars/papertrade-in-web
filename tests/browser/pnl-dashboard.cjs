...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true});try{
  ...
  await page.getByRole('button',{name:'P&L product',exact:true}).click();await page.getByRole('option',{name:'All products',exact:true}).click();await page.getByRole('button',{name:/2026-09-05:/}).click();assert.equal(await page.getByTestId('pnl-count').textContent(),'1');await page.locator('.pnl-day-detail button').click();assert.equal(await page.locator('.pnl-trade-row').count(),1);await page.getByRole('button',{name:'Clear selected day'}).click();assert.equal(await page.locator('.pnl-trade-row').count(),14);
  await page.getByRole('button',{name:'Home',exact:true}).last().click();await page.getByRole('button',{name:'Open trading coach'}).click();await page.getByRole('tab',{name:'insights',exact:true}).click();await page.getByRole('button',{name:'Open P&L insights'}).click();await page.getByRole('heading',{name:'How the rupees add up'}).waitFor();assert.equal(await page.getByRole('tab',{name:'Insights',exact:true}).getAttribute('aria-selected'),'true');
