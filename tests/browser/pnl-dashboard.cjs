...
(async()=>{
 ...
 const browser=await chromium.launch({headless:true});try{
  ...
  await page.getByLabel('P&L product').selectOption('INTRADAY');assert.equal(await page.locator('.pnl-trade-row').count(),7);await page.getByRole('tab',{name:'Overview',exact:true}).click();assert.equal(await page.getByTestId('pnl-count').textContent(),'7');
