...
(async()=>{
 ...
 await page.locator('.home-pulse-section').waitFor();assert.equal(await page.getByText('Build skill before you risk capital.',{exact:true}).count(),0);
 ...
 await page.reload();await page.locator('.home-pulse-section').waitFor();await page.getByRole('button',{name:'Show balances on Home'}).waitFor();
 assert.equal(await page.locator('.home-portfolio-value b').textContent(),'••••');
 ...
 await page.reload();await page.locator('.home-pulse-section').waitFor();await page.getByLabel('Search stocks and indices',{exact:true}).click();
