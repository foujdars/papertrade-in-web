    ...
    <section className="market-discovery-panel fno-lists-panel" aria-label="Indices and F&O lists">
      ...
      <div className="market-discovery-list">
        {filtered.map((item) => {
          ...
          return (
            <div key={item.instrumentKey} className={`trend-stock-row fno-symbol-row ${item.underlyingType === "INDEX" ? "index-row" : ""}`} role="button" tabIndex={0} onClick={() => onSelect(item)} onKeyDown={(event) => {
