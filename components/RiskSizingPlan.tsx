...
export function RiskSizingPlan(props: Props) {
  return <section className={`ticket-risk-sizing ${props.open ? "open" : ""}`}>
    ...
    {props.open && <div className="ticket-risk-body">
      ...
      <div className="ticket-risk-summary"><span>Risk<b className="negative">{formatInr(props.risk)}</b></span><span>Potential reward<b className="positive">{formatInr(props.reward)}</b></span><span>Reward : risk<b>{props.ratio ? `${props.ratio.toFixed(2)} : 1` : "—"}</b></span></div>
