...
export function TradeExecutionSummary({ trade, exitOrder, status = "Complete" }: { trade: ClosedPaperTrade; exitOrder?: PaperOrder; status?: string }) {
  ...
  return <>
    <div className="pnl-execution-pair">
      <span title={`Entry · ${time(trade.openedAt)}`}><b className={trade.direction === "LONG" ? "positive" : "negative"}>{trade.direction === "LONG" ? "BUY" : "SELL"}</b><strong>{trade.quantity} @ {formatInr(trade.entryPrice)}</strong><small><Clock3 size={12} />{compactTime(trade.openedAt)}</small></span>
      <span title={`Exit · ${time(trade.closedAt)}`}><b className={trade.direction === "LONG" ? "negative" : "positive"}>{trade.direction === "LONG" ? "SELL" : "BUY"}</b><strong>{trade.quantity} @ {formatInr(trade.exitPrice)}</strong><small><Clock3 size={12} />{compactTime(trade.closedAt)}</small></span>
    </div>
