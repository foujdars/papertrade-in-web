...
import { gmpTone, listingReturn, stageLabels, type IpoStage } from "@/lib/ipo-lifecycle";
...
export function IpoLifecycleCard({ ipo, stage, directory, onOpen }: Props) {
  ...
  return <article className="ipo-lifecycle-card" onClick={event => { if (onOpen && !(event.target as HTMLElement).closest("a,button")) onOpen(); }}>
    <header>
      ...
      <div className="ipo-card-identity"><h3>{onOpen ? <button onClick={onOpen}>{ipo.name}<ChevronRight size={16} /></button> : ipo.name}</h3><p>{ipo.symbol} · {ipo.industry || (ipo.issueType === "sme" ? "SME" : "Mainboard")}</p></div>
    ...
    <button type="button" className="ipo-card-body" onClick={onOpen} disabled={!onOpen} aria-label={`View ${ipo.name} timeline and details`}>
      {stage === "listed" ? <div className={`ipo-gmp-panel ${returns === null ? "pending" : returns >= 0 ? "high" : "low"}`}><span>Listing price<small>{ipoDate(ipo.details?.listingDate)}</small></span><strong>{rupees(ipo.details?.listingPrice)}<small>{returns === null ? "Return awaiting issue price" : `${returns > 0 ? "+" : ""}${returns.toFixed(2)}% on issue price`}</small></strong></div>
        : <div className={`ipo-gmp-panel ${tone}`}><span>Grey Market<br />Premium (GMP)<small>{ipo.gmpAmount !== null ? `${rupees(ipo.gmpAmount)} per share · unofficial` : "Unofficial market estimate"}</small></span><strong>{ipo.gmpPercent === null ? "Not reported" : `${ipo.gmpPercent.toFixed(2)}%`}</strong></div>}
      ...
      {stage !== "listed" && <div className="ipo-date-pair"><span><small>IPO opens</small><b>{ipoDate(ipo.biddingStartDate)}</b></span><span><small>IPO closes</small><b>{ipoDate(ipo.biddingEndDate)}</b></span></div>}
...
export function IpoDetailView({ ipo, stage, directory, onClose }: Props & { onClose: () => void }) {
  ...
  return <section className="ipo-detail-view" aria-label={`${ipo.name} details`}>
    <div className="ipo-detail-heading"><button onClick={onClose} className="ipo-back"><ArrowLeft size={18} /> All IPOs</button><h2 ref={heading} tabIndex={-1}>{ipo.name}</h2></div>
    ...
    <section className="ipo-detail-section"><h3><CalendarDays size={21} /> IPO tentative timeline</h3><ol className="ipo-timeline">{timeline.map(([label, date]) => <li key={label} className={(stage === "open" && label === "Close date") || (stage === "waiting" && label === "Allotment date") || (stage === "allotted" && label === "Listing date") ? "current" : ""}><span>{label}</span><b>{ipoDate(date, true)}</b></li>)}</ol><p>Dates are tentative until confirmed. An expected allotment date is not proof that results are out.</p></section>
    ...
    <section className="ipo-detail-section ipo-registrar"><h3><ShieldCheck size={21} /> Check with the registrar</h3><b>{ipo.details?.registrarName || "Registrar details awaiting confirmation"}</b>{ipo.details?.registrarUrl && <a href={ipo.details.registrarUrl} target="_blank" rel="noopener noreferrer">{stage === "allotted" ? "Allotment out — check status" : "Visit official status page"}<ArrowUpRight size={17} /></a>}<p>Enter your PAN only on the official website. PaperTrade never asks for or stores it. If publication cannot be verified, check with the registrar directly.</p></section>
  </section>;
}
